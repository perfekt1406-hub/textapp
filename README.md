...
- **Node.js 20+** (enable **Corepack** so the repo-pinned **pnpm** is used: `corepack enable`)
...
| `PORT` | HTTP signaling port for `pnpm run dev:signaling` and for the embedded host in `textr` / desktop (default **8787**). Same value on every peer on the LAN. |
...
Removes later with: `pnpm unlink --global @textr/cli`.
...
```bash
pnpm --filter @textr/desktop icons
```
...
**Installers:** `pnpm run desktop:dist` builds for the current platform; `pnpm run desktop:dist:lw` builds Linux + Windows from Linux; macOS packaging needs a Mac or the **Desktop distributables** GitHub Actions workflow.
